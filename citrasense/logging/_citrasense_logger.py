...
class _ShortNameFormatter(logging.Formatter):
    """Base formatter that strips the 'citrasense.' prefix from logger names."""
    ...
    def format(self, record):
        ...
        elif record.name == "citrasense":
            record.name = "citrasense"
...
CITRASENSE_LOGGER = logging.getLogger("citrasense")
CITRASENSE_LOGGER.setLevel(logging.INFO)
# Don't bubble up to the root logger — keeps third-party library logs separate.
CITRASENSE_LOGGER.propagate = False
...
CITRASENSE_LOGGER.handlers.clear()
CITRASENSE_LOGGER.addHandler(handler)
...
def setup_file_logging(log_file_path: Path, backup_count: int = 30) -> None:
    ...
    if _file_handler is not None:
        CITRASENSE_LOGGER.removeHandler(_file_handler)
        _file_handler.close()
        _file_handler = None
    ...
    # Add to logger
    CITRASENSE_LOGGER.addHandler(_file_handler)
