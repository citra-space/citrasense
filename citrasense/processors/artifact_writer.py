...
``satellite_matcher_debug.json``
    ...
    - ``tracking_mode``: "rate" or "sidereal" (elongation classification direction)
    - ``elongation_threshold``: 1.5 (a/b axis ratio boundary for star/satellite split)
    ...
    - ``source_classification``: total_sources, satellite_candidate_count,
      star_like_count, elongation_min/max/median/mean
    ...
    - ``match_details``: for each satellite-candidate source, the nearest
      prediction: source_ra/dec/elongation/mag,
    ...
    - ``reverse_match``: for each in-field prediction, the nearest detected
      ...
      within_match_radius, nearest_source_ra/dec/elongation/mag
...
if TYPE_CHECKING:
    from citrasense.processors.processor_result import AggregatedResult, ProcessingContext
